...
def orchestrate(historical, inventory, product_names):
    ...
    for product_name in product_names:
        ...
        del model
        itr = itr+1
    df = pd.DataFrame(data,columns=['Product Name','Current Inventory', "Number of days the current inventory will last starting 1st Sept 2019", "Quantity to purchase for the next six months"])
    
    #print(df)
    df.to_csv(r'output.csv')
    printProgressBar(100,100)
    print("\n\nAnalysis has been saved as output.csv\n\n")
    return df
